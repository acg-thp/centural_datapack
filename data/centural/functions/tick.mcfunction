...
#say tick
execute at @a as @a[y=-1024,dy=256,tag=in_underworld] run tag @s remove in_underworld
...
execute at @a as @a[y=-769,dy=512,tag=in_void] run tag @s remove in_void
execute at @a as @a[y=-769,dy=512,tag=!in_underworld] run tellraw @a "Welcome to the Underworld"
execute at @a as @a[y=-769,dy=512,tag=!in_underworld] run tag @s add in_underworld
