...
execute if entity @p[scores={warp=-1},tag=warp_underworld] as @a[scores={warp=-1},tag=warp_underworld] unless entity @s[scores={temporary_timer=-2147483648..2147483647}] run scoreboard objectives add temporary_timer minecraft.custom:minecraft.play_one_minute
execute if entity @p[scores={warp=-1,temporary_timer=1},tag=warp_underworld] as @a[scores={warp=-1,temporary_timer=1},tag=warp_underworld] run tellraw @s "Warping to the Underworld... Please Wait."
