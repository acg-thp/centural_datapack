...
execute if entity @p[scores={warp=-1},tag=warp_underworld] as @a[scores={warp=-1},tag=warp_underworld] run tellraw @s "Warping to the Underworld... Please Wait."
execute as @a unless entity @s[scores={temporary_timer=-2147483648..2147483647}] run scoreboard objectives add temporary_timer minecraft.custom:minecraft.play_one_minute
execute if entity @p[tag=warp_underworld,scores={temporary_timer=150..}] as @a[tag=warp_underworld,scores={temporary_timer=150..}] run tp @s 0 -768 0
...
execute if entity @p[scores={temporary_timer=150..}] as @a[scores={temporary_timer=150..}] run scoreboard objectives remove temporary_timer
