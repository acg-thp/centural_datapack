execute if entity @e[tag=elevator,type=armor_stand] at @e[tag=elevator,type=armor_stand] at @e[tag=elevator_slab,distance=..1,limit=1] align x align y align z positioned ~.5 ~ ~.5 run particle minecraft:heart ~ ~ ~ 0 0 0 1 1 force @a[distance=..16]

execute if entity @e[tag=elevator,type=armor_stand] at @e[tag=elevator,type=armor_stand] at @e[tag=elevator_slab,distance=..1,limit=1] align x align y align z positioned ~.5 ~ ~.5 if entity @p[distance=..1,tag=!elevator_up] as @a[distance=..1,tag=!elevator_up] run tag @s add elevator_up 
...
execute if entity @p[tag=elevator_up] as @a[tag=elevator_up] at @s anchored feet align x align y align z positioned ~ ~ ~ if entity @e[tag=elevator_slab,dx=1,dy=1,dz=1,type=falling_block] run effect give @s minecraft:levitation 1 1


execute if entity @p[tag=elevator_up] as @a[tag=elevator_up] at @s anchored feet align x align y align z positioned ~ ~ ~ if entity @e[tag=elevator_slab,dx=1,dy=1,dz=1,type=falling_block] run effect give @e[tag=elevator,limit=1,distance=..3,type=armor_stand] minecraft:levitation 1 1
