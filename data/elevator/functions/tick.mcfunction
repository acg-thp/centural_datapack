...
execute if entity @p[tag=elevator_up] as @a[tag=elevator_up] at @s anchored feet align x align y align z positioned ~ ~-1 ~ if entity @e[tag=elevator_slab,dx=1,dy=1,dz=1,type=falling_block] run run particle minecraft:heart ~ ~ ~ 0 0 0 1 1 force @a[distance=..16]
