...
execute if entity @p[tag=elevator_up] as @a[tag=elevator_up] at @s anchored feet align x align y align z positioned ~ ~ ~ if entity @e[tag=elevator,dx=1,dy=1,dz=1,type=armor_stand] at @e[tag=elevator,dx=1,dy=1,dz=1,type=armor_stand] align x align y align z positioned ~.5 ~ ~.5 unless entity @s[distance=..2] run tp @s ~ ~2 ~
